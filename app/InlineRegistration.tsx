...
export function InlineRegistration() {
  ...
  return <section id="pre-matricula" className="inline-registration section"><div><p className="eyebrow">PRÉ-INSCRIÇÃO · GRATUITA</p><h2>Sua próxima<br/><em>jornada começa aqui.</em></h2><p>Deixe seus dados e escolha o curso de seu interesse.</p></div><form onSubmit={submit}><label>Nome<input required name="nome" autoComplete="name" placeholder="Seu Nome" /></label><label>Telefone<input required name="telefone" inputMode="tel" autoComplete="tel" placeholder="(21) 00000-0000" /></label><label>E-mail<input required type="email" name="email" autoComplete="email" placeholder="voce@email.com" /></label><label>Curso de interesse<select required name="curso" defaultValue=""><option value="" disabled>Escolha uma opção</option><option value="robotica">Robótica</option><option value="games">Desenvolvimento de Games</option><option value="informatica60">Inclusão Digital 60+</option><option value="ingles">Introdução à Língua Inglesa</option><option value="apps">Criação de Aplicativos</option><option value="musica">Música</option></select></label>{error && <p role="alert" aria-live="polite">{error}</p>}<button className="button" type="submit" disabled={sending}>{sending ? "Enviando…" : "Enviar pré-inscrição"} <span>↗</span></button></form></section>;
}
