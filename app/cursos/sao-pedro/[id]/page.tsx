...
export default async function SaoPedroCoursePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="course-detail-page">
    ...
    <section className="course-detail-content"><div className="course-detail-copy"><p className="eyebrow">O QUE VOCÊ VAI APRENDER</p><h2>Aprender fazendo<br/><em>abre caminhos.</em></h2><p>{course.learning}</p><p>As atividades são pensadas para o ritmo da turma, com prática, troca e acompanhamento da equipe.</p></div><div className="course-detail-info"><div><span>FAIXA ETÁRIA</span><strong>{course.age}</strong></div><div className="course-schedules"><span>TURMAS E HORÁRIOS</span>{course.schedules.map((schedule) => <strong key={schedule}>{schedule}</strong>)}</div><a className="button" href={`/pre-matricula?unidade=sao-pedro&curso=${course.id}`}>Fazer pré-inscrição <span>↗</span></a></div></section>
  </main>;
}
