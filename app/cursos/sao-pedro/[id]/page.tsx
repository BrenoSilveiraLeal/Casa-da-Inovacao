...
export default async function SaoPedroCoursePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="course-detail-page">
    ...
    <section className="course-detail-hero"><p className="eyebrow">SÃO PEDRO · {course.category}</p><h1>{course.id === "games" ? <>Desenvolvi-<br />mento de Games</> : course.name}</h1><p className="course-detail-intro">{course.description}</p></section>
